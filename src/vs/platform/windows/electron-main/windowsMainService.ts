...
export class WindowsMainService extends Disposable implements IWindowsMainService {
	...
	private async doOpen(
		...
		if (filesToOpen && potentialNewWindowsCount === 0) {
			...
			// Finally, if no window or folder is found, just open the files in an empty window
				...
				const sessionsWindow = !openFilesInNewWindow ? this.getSessionsWindowForFolderHandoff(openConfig) : undefined;
				if (sessionsWindow) {
	...
	private getLastActiveNonSessionsWindow(): ICodeWindow | undefined {
		return this.doGetLastActiveWindow(this.getWindows().filter(window => !window.config?.isSessionsWindow));
	}

	private getSessionsWindowForFolderHandoff(openConfig: IOpenConfiguration): ICodeWindow | undefined {
		if (openConfig.initialStartup) {
			return undefined;
		}
		...
		return lastActiveWindow?.config?.isSessionsWindow ? lastActiveWindow : undefined;
	}

	private handoffFolderToSessionsWindow(window: ICodeWindow, folderUri: URI): void {
		this.logService.trace('windowsManager#handoffFolderToSessionsWindow', { folderUri: folderUri.toString(), windowId: window.id });
		window.focus();
		window.sendWhenReady('vscode:selectAgentsFolder', CancellationToken.None, folderUri.toJSON(), undefined, AgentsWindowOpenSource.Unknown);
	}
