...
import { CodexSessionConfigKey, collaborationModeKind, inferCodexPermissionsPreset, migrateCodexPermissionValues, narrowAdditionalDirectories, narrowApprovalPolicy, narrowBoolean, narrowCodexPermissionsPreset, narrowPersonality, narrowReasoningEffort, narrowReasoningSummary, narrowSandboxMode, narrowWebSearchMode, presetForResolvedPermissions, resolveCodexPermissions, resolveCodexPermissionsPreset } from '../../../node/codex/codexSessionConfigKeys.js';
...
suite('codexSessionConfigKeys', () => {
	...
	test('infers the user-facing preset from config.toml security axes', () => {
		assert.deepStrictEqual({
			default: inferCodexPermissionsPreset('on-request', 'workspace-write', 'user'),
			autoReview: inferCodexPermissionsPreset('on-request', 'workspace-write', 'auto_review'),
			fullAccess: inferCodexPermissionsPreset('never', 'danger-full-access', 'user'),
			unset: inferCodexPermissionsPreset(undefined, undefined, undefined),
			dangerSandbox: inferCodexPermissionsPreset('on-request', 'danger-full-access', 'user'),
		}, {
			default: 'default',
			autoReview: 'auto-review',
			fullAccess: 'full-access',
			unset: 'default',
			dangerSandbox: 'full-access',
		});
	});
