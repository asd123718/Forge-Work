...
/** Narrow an arbitrary runtime value to {@link CodexApprovalsReviewer}. */
export function narrowCodexApprovalsReviewer(raw: unknown): CodexApprovalsReviewer | undefined {
	switch (raw) {
		case 'user':
		case 'auto_review':
...
/**
 * Map persisted Codex config.toml security axes onto a user-facing preset.
 * Unknown or unmatched combinations fall back to {@link CODEX_DEFAULT_PERMISSIONS_PRESET},
 * except a `danger-full-access` sandbox which maps to Full Access.
 */
export function inferCodexPermissionsPreset(approvalPolicy: unknown, sandboxMode: unknown, approvalsReviewer: unknown): CodexPermissionsPreset {
	const resolved: ICodexResolvedPermissions = {
	...
	return presetForResolvedPermissions(resolved)
		?? (resolved.sandboxMode === 'danger-full-access' ? 'full-access' : CODEX_DEFAULT_PERMISSIONS_PRESET);
}
