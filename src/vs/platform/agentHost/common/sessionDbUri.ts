 ...
 */
export function buildSessionDbUri(sessionUri: string, toolCallId: string, filePath: string, part: 'before' | 'after', revision?: number): string {
...
export interface ISessionDbUriFields {
	...
	filePath: string;
	part: 'before' | 'after';
	/** Monotonic revision for an in-flight streaming preview. */
	revision?: number;
}
...
function parseSessionDbUriQuery(query: string): ISessionDbUriFields | undefined {
	...
	return revision === undefined
		? { sessionUri, toolCallId, filePath, part }
		: { sessionUri, toolCallId, filePath, part, revision };
}
