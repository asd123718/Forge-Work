...
export function getRenameChatArgs(rawArgs: unknown, sessions: readonly IAgentSessionMetadata[], currentChannel?: ProtocolURI): IResolvedRenameChatArgs {
	...
	// Codex runs server tools under the session/config scope (`codex:/…`), not
	// the `ahp-chat://` channel. Treat that scope — or an explicit session
	// argument without `chat` — as "rename this session's default chat".
	const explicitSession = sessionInput !== undefined ? resolveKnownSession(sessionInput, sessions) : undefined;
	if (sessionInput !== undefined && !explicitSession) {
		throw new Error(`Invalid ${SessionServerToolName.RenameChat} input: session must match the URI of a known session (see list_sessions).`);
	}
	const sessionFromChannel = currentChannel ? currentSessionUri(currentChannel) : undefined;
