	...
	async setLocale(languagePackItem: ILanguagePackItem, skipDialog = false): Promise<void> {
		...
		try {
			...
			if (!installedLanguages.some(installedLanguage => installedLanguage.id === languagePackItem.id)) {
				...
				const isBundledForgePack = languagePackItem.extensionId === 'forge.forge-language-pack-zh-hans'
					|| !!languagePackItem.extensionId?.startsWith('forge.');

				// Only actually install a language pack from Microsoft. Forge ships its
				// own Chinese pack as a built-in extension and should not bounce to the
				// Marketplace when that pack is not yet in languagepacks.json.
