...
async function openCodexSettings(accessor: ServicesAccessor): Promise<void> {
...
/**
 * Renders the "Open Codex Settings" titlebar entry as an icon-only button that
...
export class AgentsHandoffInputTipContribution extends Disposable implements IWorkbenchContribution {
	...
	private _update(): void {
		...
		const message = useEmptyWorkspaceCopy
			...
			: localize('chat.agentsHandoff.tip.message', "Configure this session in Codex Settings");
		const description = useEmptyWorkspaceCopy
			? localize('chat.agentsHandoff.tip.emptyWorkspace.description', "Open Codex Settings to configure the agent runtime.")
		...
		const actionLabel = useEmptyWorkspaceCopy
			? localize('chat.agentsHandoff.tip.action', "Open Codex Settings")
			: mode === AgentsHandoffTipMode.Custom
				? localize('chat.agentsHandoff.tip.action.custom', "Give your agent more room?")
				: localize('chat.agentsHandoff.tip.action.default', "Open Codex Settings");
