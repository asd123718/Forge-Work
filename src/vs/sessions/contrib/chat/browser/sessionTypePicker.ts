...
export class SessionTypePicker extends Disposable {
	...
	protected _computeCurrentPick(): IPreferredSessionType | undefined {
		...
		return this._getPreferredCodexPick()
			?? (preferred ? { providerId: preferred.providerId, sessionTypeId: preferred.sessionType.id } : undefined);
	}
	...
	private _getPreferredCodexPick(): IPreferredSessionType | undefined {
		const codex = this._folderSessionTypes.find(type => type.sessionType.id === CODEX_AGENT_PROVIDER_ID);
		return codex ? { providerId: codex.providerId, sessionTypeId: codex.sessionType.id } : undefined;
	}
