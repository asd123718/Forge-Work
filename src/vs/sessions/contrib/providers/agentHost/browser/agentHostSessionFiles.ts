...
export interface IParsedFileEdit {
	...
	/** After-content snapshot URI, revised as an in-flight edit streams. */
...
interface IMutableTurnChange {
	...
	modifiedSnapshotUri: URI | undefined;
...
export function reduceTurnChanges(
	...
	const setCreated = (uri: URI, modifiedSnapshotUri: URI | undefined, insertions: number, deletions: number): void => {
		...
		byUri.set(key, { uri, modifiedUri: uri, modifiedSnapshotUri, originalUri: undefined, isOutsideWorkspace: isOutsideWorkspace(uri), created: true, insertions, deletions });
	};

	const setModified = (uri: URI, originalUri: URI | undefined, modifiedSnapshotUri: URI | undefined, insertions: number, deletions: number): void => {
		const key = getComparisonKey(uri);
		const existing = byUri.get(key);
		if (existing) {
			existing.insertions += insertions;
	...
	const setDeleted = (uri: URI, originalUri: URI | undefined, modifiedSnapshotUri: URI | undefined, insertions: number, deletions: number): void => {
		const key = getComparisonKey(uri);
		if (byUri.has(key)) {
		...
		byUri.set(key, { uri, modifiedUri: undefined, modifiedSnapshotUri, originalUri, isOutsideWorkspace: isOutsideWorkspace(uri), created: false, insertions, deletions });
	};
