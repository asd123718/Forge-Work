...
BUILD_TARGETS.forEach(buildTarget => {
	...
	const destinationExe = path.join(root, '.build', 'scanbin', `VSCode${dashed(platform)}${dashed(arch)}`, 'bin');
	const destinationPdb = path.join(root, '.build', 'scanbin', `VSCode${dashed(platform)}${dashed(arch)}`, 'pdb');
