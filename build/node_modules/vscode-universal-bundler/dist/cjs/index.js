...
const makeUniversalApp = async (opts) => {
    ...
    const x64AsarMode = await (0, asar_utils_1.detectAsarMode)(opts.x64AppPath, opts.asarPath);
    const arm64AsarMode = await (0, asar_utils_1.detectAsarMode)(opts.arm64AppPath, opts.asarPath);
    ...
    try {
        ...
        for (const file of x64Files.filter((f) => f.type === file_utils_1.AppFileType.PLAIN)) {
            ...
            if (x64Sha !== arm64Sha) {
                ...
                if (opts.filesToSkipComparison) {
                    if (typeof opts.filesToSkipComparison === 'function' &&
        ...
        for (const machOFile of x64Files.filter((f) => f.type === file_utils_1.AppFileType.MACHO)) {
            ...
            if ((await (0, asar_utils_1.isUniversalMacho)(first)) && (await (0, asar_utils_1.isUniversalMacho)(second))) {
                (0, debug_1.d)('Skipping universal Mach-O file', machOFile.relativePath);
        ...
        if (x64AsarMode === asar_utils_1.AsarMode.HAS_ASAR && opts.mergeASARs) {
            ...
            generatedIntegrity[(_a = opts.asarPath) !== null && _a !== void 0 ? _a : 'Resources/app.asar'] = (0, asar_utils_1.generateAsarIntegrity)(output);
        }
        else if (x64AsarMode === asar_utils_1.AsarMode.HAS_ASAR) {
            (0, debug_1.d)('checking if the x64 and arm64 asars are identical');
            const x64AsarSha = await (0, sha_1.sha)(opts.asarPath
                ? path.resolve(tmpApp, opts.asarPath)
                : path.resolve(tmpApp, 'Contents', 'Resources', 'app.asar'));
            const arm64AsarSha = await (0, sha_1.sha)(opts.asarPath
                ? path.resolve(opts.arm64AppPath, opts.asarPath)
                : path.resolve(opts.arm64AppPath, 'Contents', 'Resources', 'app.asar'));
        ...
        for (const plistFile of plistFiles) {
            ...
            const _b = plist.parse(await fs.readFile(x64PlistPath, 'utf8')), { ElectronAsarIntegrity: x64Integrity } = _b, x64Plist = __rest(_b, ["ElectronAsarIntegrity"]);
            const _c = plist.parse(await fs.readFile(arm64PlistPath, 'utf8')), { ElectronAsarIntegrity: arm64Integrity } = _c, arm64Plist = __rest(_c, ["ElectronAsarIntegrity"]);
