...
import { AsarMode, detectAsarMode, generateAsarIntegrity, mergeASARs, isUniversalMacho, } from './asar-utils';
import { sha } from './sha';
import { d } from './debug';
...
export const makeUniversalApp = async (opts) => {
    ...
    try {
        ...
        if (x64AsarMode === AsarMode.HAS_ASAR && opts.mergeASARs) {
            ...
            generatedIntegrity[(_a = opts.asarPath) !== null && _a !== void 0 ? _a : 'Resources/app.asar'] = generateAsarIntegrity(output);
        }
        else if (x64AsarMode === AsarMode.HAS_ASAR) {
            d('checking if the x64 and arm64 asars are identical');
            const x64AsarSha = await sha(opts.asarPath
                ? path.resolve(tmpApp, opts.asarPath)
                : path.resolve(tmpApp, 'Contents', 'Resources', 'app.asar'));
            const arm64AsarSha = await sha(opts.asarPath
                ? path.resolve(opts.arm64AppPath, opts.asarPath)
                : path.resolve(opts.arm64AppPath, 'Contents', 'Resources', 'app.asar'));
