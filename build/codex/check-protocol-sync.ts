...
function readCodexPackageVersion(): string {
	const pkg = JSON.parse(readFileSync(path.join(REPO_ROOT, 'package.json'), 'utf8'));
	const version: string | undefined = pkg.dependencies?.['@openai/codex'] ?? pkg.devDependencies?.['@openai/codex'];
	if (!version) {
		throw new Error('Could not find @openai/codex in repo-root package.json dependencies.');
	}
	return version.trim();
}
...
function main(): void {
	...
	if (binVersion !== devDep) {
		fail(`The vendored codex binary is ${binVersion} but package.json pins @openai/codex ${devDep}. ` +
			`Run \`npm ci\` so the freshness check regenerates with the dependency version.`);
	}
	...
	try {
		...
		if (diffs.length > 0) {
			...
			console.error(`  (ensure ${VERSION_FILE_REL} and the @openai/codex dependency are both ${devDep} first.)`);
			process.exit(1);
		}
	} finally {
