...
function getEnumType(type) {
    ...
    for (const declaration of declarations) {
        if (declaration.getKind() === ts_morph_1.SyntaxKind.EnumDeclaration) {
            const enumDeclaration = declaration;
            const members = enumDeclaration.getChildrenOfKind(ts_morph_1.SyntaxKind.EnumMember);
...
class TsParser {
    ...
    addEventDefinition(eventName, properties, location) {
    ...
    extractConflictProperties(eventProperties) {
        const result = {};
    ...
    parseFiles() {
        ...
        this.project.getSourceFiles().forEach((source) => {
            const descendants = source.getDescendantsOfKind(ts_morph_1.SyntaxKind.CallExpression).filter((c) => c.getExpression().getText().includes('publicLog2') && c.getArguments().length > 0);
            const descendants2 = source.getDescendantsOfKind(ts_morph_1.SyntaxKind.CallExpression).filter((c) => c.getExpression().getText().includes('publicLogError2') && c.getArguments().length > 0);
