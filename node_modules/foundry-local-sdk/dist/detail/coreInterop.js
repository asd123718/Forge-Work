...
function loadAddon() {
    ...
    // VSCODE_PATCH:foundry-addon-native-dir: prefer the on-demand native runtime cache when present.
...
export class CoreInterop {
    ...
    static _resolveDefaultCorePath(config) {
        ...
        const arch = process.arch;
        const platformKey = `${platform}-${arch}`;
        // VSCODE_PATCH:foundry-core-native-dir: prefer the on-demand native runtime cache when present.
        const overrideDir = process.env.VSCODE_FOUNDRY_LOCAL_NATIVE_DIR;
        if (overrideDir) {
