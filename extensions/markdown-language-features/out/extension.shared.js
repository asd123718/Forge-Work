...
function activateShared(context, client, engine, logger, contributions) {
    ...
    const linkPresentationService = (0, linkPresentationService_1.createSharedLinkPresentationService)(context.globalState, logger);
    context.subscriptions.push(linkPresentationService, (0, linkPresentationService_1.registerLinkPresentationProvider)(linkPresentationService), vscode.window.onDidChangeWindowState(event => {
        if (event.focused) {
            linkPresentationService.refresh();
        }
    }));
    const markdownEditorProvider = new markdownEditorProvider_1.MarkdownEditorProvider(context.extensionUri, context.globalState, opener, contributions, logger, href => linkPresentationService.openLink(href));
    ...
    context.subscriptions.push(vscode.window.registerCustomEditorProvider(markdownEditorProvider_1.MarkdownEditorProvider.viewType, markdownEditorProvider, {
...
function registerMarkdownEditorCommands(context, provider) {
    const contributions = context.extension.packageJSON.contributes;
