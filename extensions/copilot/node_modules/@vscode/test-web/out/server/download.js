...
async function downloadAndUntar(downloadUrl, destination, message) {
    ...
    const tar = await Promise.resolve().then(() => __importStar(require('tar-fs')));
    const gunzip = await Promise.resolve().then(() => __importStar(require('gunzip-maybe')));
    return new Promise((resolve, reject) => {
        ...
        httpLibrary.get(downloadUrl, getAgent(downloadUrl), res => {
            ...
            const extract = res.pipe(gunzip.default()).pipe(tar.extract(destination, { strip: 1 }));
            extract.on('finish', () => {
