...
function runBrew(label) {
  const result = (0, import_child_process.spawnSync)("brew", ["install", "copilot-cli"], { stdio: "inherit", env: env2 });
  if (result.error) {
...
function runCurl(label) {
  const result = (0, import_child_process.spawnSync)("bash", ["-c", "curl -fsSL https://gh.io/copilot-install | bash"], { stdio: "inherit", env: env2 });
  if (result.error) {
...
function runWget(label) {
  const result = (0, import_child_process.spawnSync)("bash", ["-c", "wget -qO- https://gh.io/copilot-install | bash"], { stdio: "inherit", env: env2 });
  if (result.error) {
...
function hasCommand(cmd) {
  const result = (0, import_child_process.spawnSync)("sh", ["-c", `command -v ${cmd}`], { env: env2, encoding: "utf8" });
  return !result.error && result.status === 0;
}
function installCopilotCLI(label, update = false) {
