...
suite('git smoke test', function () {
    ...
    test.skip('opens notebook diff and file from active notebook editor', async function () {
        ...
        await repository.status();
        try {
            const notebook = await vscode_1.workspace.openNotebookDocument(uri('notebook.ipynb'));
            await vscode_1.window.showNotebookDocument(notebook);
            // git.openChange without an argument resolves the resource from the active notebook editor
            await vscode_1.commands.executeCommand('git.openChange');
            (0, assert_1.default)(vscode_1.window.tabGroups.activeTabGroup.activeTab?.input instanceof vscode_1.TabInputNotebookDiff);
            // git.openFile toggles back to the notebook from the active notebook diff editor
            await vscode_1.commands.executeCommand('git.openFile');
            (0, assert_1.default)(vscode_1.window.tabGroups.activeTabGroup.activeTab?.input instanceof vscode_1.TabInputNotebook);
        }
        finally {
