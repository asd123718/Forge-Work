...
export class TelemetryReporter extends BaseTelemetryReporter {
    constructor(connectionString, replacementOptions) {
        let clientFactory = (connectionString) => appInsightsClientFactory(connectionString, vscode.env.machineId, vscode.env.sessionId, getXHROverride(), replacementOptions);
        // If connection string is usable by 1DS use the 1DS SDk
        if (TelemetryUtil.shouldUseOneDataSystemSDK(connectionString)) {
            clientFactory = (key) => oneDataSystemClientFactory(key, vscode, getXHROverride());
        }
        const osShim = {
            release: os.release(),
