...
(vscode.env.uiKind === vscode.UIKind.Web ? suite.skip : suite)('chat - run_in_terminal', () => {
    let disposables = [];
    let originalShellIntegrationEnabled;
    let windowsUseConptyDll;
    setup(async () => {
        disposables = [];
        ...
        originalShellIntegrationEnabled = terminalConfig.get('shellIntegration.enabled');
        await terminalConfig.update('shellIntegration.enabled', true, vscode.ConfigurationTarget.Global);
        if (isWindows) {
            windowsUseConptyDll = terminalConfig.get('windowsUseConptyDll');
        ...
        const testLanguageModelProvider = {
        ...
        disposables.push(vscode.lm.registerLanguageModelChatProvider('test-lm-vendor', testLanguageModelProvider), vscode.lm.registerLanguageModelChatProvider('copilot', testLanguageModelProvider));
    ...
    async function invokeRunInTerminal(command, optionsOrTimeout) {
        setupParticipant();
        const opts = typeof optionsOrTimeout === 'number'
            ? { timeout: optionsOrTimeout }
            : optionsOrTimeout ?? {};
        const resultPromise = new utils_1.DeferredPromise();
    ...
    (isWindows ? suite.skip : suite)('sandbox on', () => {
        ...
        test.skip('can read files outside the workspace', async function () {
            ...
            assert.ok(lastLine.startsWith('#'), `Expected a comment line from /etc/shells, got: ${trimmed}`);
        });
...
//# sourceMappingURL=chat.runInTerminal.test.js.map